...
openssl req -new -x509  -days 3650 -sha256 -key server.key  -out server.crt -subj "/C=cn/OU=custer/O=custer/CN=192.168.209.8" -config ${opensslCnfFile} -extensions v3_req
...
openssl req -new -x509 -days 3650 -key ca.key -out ca.pem -subj "/C=cn/OU=custer/O=custer/CN=192.168.209.8"

# server
openssl genpkey -algorithm RSA -out server.key
openssl req -new -nodes -key server.key -out server.csr -days 3650 -subj "/C=cn/OU=custer/O=custer/CN=192.168.209.8" -config ${opensslCnfFile} -extensions v3_req
...
openssl req -new -nodes -key client.key -out client.csr -days 3650 -subj "/C=cn/OU=custer/O=custer/CN=192.168.209.8" -config ${opensslCnfFile} -extensions v3_req
