...
func TestError(t *testing.T) {
	...
	req.SetURL("http://192.168.209.8:0")
	...
	assert.Error(t, err)
	err = do(http.MethodPost, &StdResult{}, "http://192.168.209.8:0", nil, KV{"foo": "bar"})
	assert.Error(t, err)

	err = gDo(http.MethodGet, nil, "http://192.168.209.8:0", nil)
	assert.Error(t, err)
}
