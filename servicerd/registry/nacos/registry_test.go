...
import (
	...
	"github.com/18721889353/common_pkg/utils"
...
func TestRegistry_RegisterError(t *testing.T) {
	instance := registry.NewServiceInstance("", "", []string{"grpc://192.168.209.8:8282"})
	r := &Registry{}
	utils.SafeRunWithTimeout(time.Second*3, func(cancel context.CancelFunc) {
		r = newNacosRegistry()
	})
	...
	instance = registry.NewServiceInstance("foo", "bar", []string{"192.168.209.8:port"})
	err = r.Register(context.Background(), instance)
	assert.Error(t, err)

	instance = registry.NewServiceInstance("foo", "bar", []string{"192.168.209.8"})
	err = r.Register(context.Background(), instance)
	assert.Error(t, err)
}
