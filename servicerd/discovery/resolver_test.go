...
func Test_discoveryResolver_Close(t *testing.T) {
	...
	r.update([]*registry.ServiceInstance{registry.NewServiceInstance(
		"foo",
		"bar",
		[]string{"grpc://192.168.209.8:8282"},
	)})
	//r.watch()
...
func Test_parseEndpoint(t *testing.T) {
	_, err := parseEndpoint([]string{"grpc://192.168.209.8:8282"}, "grpc", false)
	assert.NoError(t, err)
	_, err = parseEndpoint([]string{"grpc://192.168.209.8:8282"}, "grpc", true)
	assert.NoError(t, err)
...
func TestIsSecure(t *testing.T) {
	u, err := url.Parse("http://192.168.209.8:8080")
	assert.NoError(t, err)

	ok := IsSecure(u)
	assert.Equal(t, false, ok)

	u, _ = url.Parse("http://192.168.209.8:8080?isSecure=true")
	ok = IsSecure(u)
